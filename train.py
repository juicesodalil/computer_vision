...
train_loader = data.DataLoader(train_set_10, batch_size=batch_size, shuffle=True)
test_loader = data.DataLoader(test_set_10, batch_size, shuffle=True)

device = torch.device("cuda")
# 得到模型
model = ResidualNetwork.ResNet18(10)
# model.to(device)
...
if is_test:
    ...
    # model.load_state_dict(torch.load(f"pretrain/cifar10-{network_name}-50.pth", map_location="cuda:0"))
    model.to(device)
